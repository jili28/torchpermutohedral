...
torch::Tensor PermutohedralFilter_forward(torch::Tensor input, torch::Tensor features) {
...
#define CASE(dc, fc)                                                                                                  \
  AT_DISPATCH_FLOATING_TYPES(data.scalar_type(), "PermutohedralCuda", ([&] {                                          \
                               for (int batchIndex = 0; batchIndex < batchCount; batchIndex++) {                      \
                                 ...
                                 PermutohedralCuda<scalar_t, dc, fc>(offsetData, offsetFeatures, elementCount, true, false); \
                               }                                                                                      \
                             }));
    ...
    AT_DISPATCH_FLOATING_TYPES(
        data.scalar_type(), "PermutohedralCPU", ([&] {
          for (int batchIndex = 0; batchIndex < batchCount; batchIndex++) {
            ...
            PermutohedralCPU<scalar_t>(offsetData, offsetFeatures, channelCount, featureCount, elementCount, false);
          }
        }));
#ifdef WITH_CUDA
  }
...
torch::Tensor PermutohedralFilter_backward(torch::Tensor input, torch::Tensor features) {
  input = input.contiguous();
...
#define CASE(dc, fc)                                                                                                  \
  AT_DISPATCH_FLOATING_TYPES(data.scalar_type(), "PermutohedralCuda", ([&] {                                          \
                               for (int batchIndex = 0; batchIndex < batchCount; batchIndex++) {                      \
                                 ...
                                 scalar_t* offsetFeatures =                                                           \
                                     features.data_ptr<scalar_t>() + batchIndex * fc * elementCount;                  \
                                 PermutohedralCuda<scalar_t, dc, fc>(offsetData, offsetFeatures, elementCount, true, true); \
                               }                                                                                      \
                             }));
    SWITCH_AB(CASE, PHL_CUDA_MAX_CHANNELS, PHL_CUDA_MAX_FEATURES, channelCount, featureCount);
    ...
    AT_DISPATCH_FLOATING_TYPES(
        data.scalar_type(), "PermutohedralCPU", ([&] {
          for (int batchIndex = 0; batchIndex < batchCount; batchIndex++) {
            scalar_t* offsetData = data.data_ptr<scalar_t>() + batchIndex * batchStride;
            scalar_t* offsetFeatures = features.data_ptr<scalar_t>() + batchIndex * featureCount * elementCount;
            PermutohedralCPU<scalar_t>(offsetData, offsetFeatures, channelCount, featureCount, elementCount, true);
          }
        }));
#ifdef WITH_CUDA
  }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  // filtering
  m.def("phl_filter_forward", &PermutohedralFilter_forward, "Permutohedral Filter forward");
  m.def("phl_filter_backward", &PermutohedralFilter_backward, "Permutohedral Filter backward");
}
